...
//Replaced in newest script
function styleTract(feature) {
  return {
    fillColor: getIncomeColor(feature.properties.income_change),
    weight: 1,
    opacity: 1,
    color: "white",
    fillOpacity: 0.5,
...
// Replaced below with new version in current script to make houses different sizes, recolor choropleth, etc. 

function addFeatures(feature, layer) {
  const props = feature.properties;
  const propChange = props.property_value_change;

  // 1. Check if historical data is missing (null) due to tract splits
  if (props.income_change === null || propChange === null) {
    const fallbackHTML = `
      <div style="font-family: sans-serif; max-width: 250px;">
        <strong>${props.Census_Tract}</strong><br>
        Neighborhood: ${props.NeighID}<br>
    ...
    return; // Stop here! Skips the icons and charts for this specific tract
  }

  // 2. Add House Icon Marker (only runs if data exists)
  if (propChange !== null && propChange !== undefined) {
    let iconHtml =
      propChange >= 0
        ? `<div style="font-size: 20px; color: #40E0D0; opacity: 0.8; text-shadow: 1px 1px 2px #000;">🏠</div>` // Turquoise
        : `<div style="font-size: 20px; color: #FFFF00; opacity: 0.8; transform: rotate(180deg); text-shadow: 1px 1px 2px #000;">🏠</div>`; // Yellow
    ...
    if (
      ...
      L.marker(center, { icon: customIcon, interactive: false }).addTo(map);
    }
  }

  // 3. Build the Standard Popup Content
  let popupContent = `<div style="min-width: 200px; font-family: sans-serif;">`;
  popupContent += `<b>Tract: ${props.Census_Tract}</b> (${props.NeighID})<br><hr style="margin: 5px 0;">`;

  const incStr =
    props.income_change !== null
      ? `$${props.income_change.toLocaleString()}`
      : "N/A";
  const propStr =
    propChange !== null ? `$${propChange.toLocaleString()}` : "N/A";

  popupContent += `💵 <b>Income</b> Change: +${incStr}<br>`;
  popupContent += `🏘️ <b>Property Value</b> Change: +${propStr}<br>`;

  // 4. Add the dynamic bar charts
...
// REPLACED below in newest js file so can do things like only show the levels
// of increases opposed to showing decreases in property value & income.
